...
const METRICS = {
  net_income: {
    ...
    methodology_pl:
      'Dochód netto właściciela to zatwierdzone wpłaty pomniejszone o koszty bezpośrednie, alokowane koszty właściciela i podatek ryczałtowy.',
  },
  revenue_paid: {
    ...
    methodology_pl:
      'Przychód z wpłat obejmuje faktyczne total_paid dla płatności opłaconych i częściowych. Płatności oczekujące i zaległe nie są przychodem zrealizowanym.',
  },
  revenue_expected: {
  ...
  expenses: {
    ...
    methodology_pl:
      'Koszty obejmują koszty bezpośrednie oraz alokowane koszty właściciela zgodnie z warstwą finance-summary.',
  },
  tax_total: {
    label_pl: 'Podatek',
    ...
    methodology_pl:
      'Podatek liczony jest miesięcznie od zatwierdzonego czynszu, z zaokrągleniem do złotówki i podatkiem stałym, jeśli skonfigurowany.',
  },
  margin: {
    ...
    methodology_pl:
      'Marża netto pokazuje, jaka część faktycznych wpłat zostaje właścicielowi po odjęciu kosztów i podatku. Liczymy ją jako netto / wpłaty. Np. marża 30% oznacza, że z każdej 1 zł wpłaty zostaje około 30 gr.',
  },
  tenant_paid_total: {
    ...
    methodology_pl:
      'Suma wpłat najemcy obejmuje faktyczne total_paid dla płatności opłaconych i częściowych w wybranym zakresie.',
  },
  tenant_balance: {
  ...
  tenant_count: {
    ...
    methodology_pl:
      'Liczba najemców to unikalni najemcy powiązani z płatnościami lub umowami w danym zakresie.',
  },
};
...
function inferMetric(question) {
  ...
  if (includesAny(text, ['czy zaplacil', 'status platnosci'])) {
    return {
      key: 'payment_status',
      metric: { label_pl: 'Status płatności' },
      ambiguous: false,
      source: 'rule',
    };
  }
  ...
  if (sameScore.length > 1 && sameScore.some((hit) => hit.key !== top.key)) {
    const defaultHit = sameScore.find(
      (hit) =>
        hit.metric.default_for &&
        hit.matched.some((alias) => hit.metric.default_for.map(normalizeText).includes(normalizeText(alias))),
    );
    if (defaultHit)
      return { key: defaultHit.key, metric: defaultHit.metric, ambiguous: false, source: 'default_alias' };
    return {
      key: top.key,
      metric: top.metric,
      ambiguous: true,
      options: sameScore.map((hit) => ({ key: hit.key, label: hit.metric.label_pl })),
    };
  }
