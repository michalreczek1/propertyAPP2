...
router.post('/', validate(ExpenseSchema), (req, res) => {
  const b = req.body;
  if (!assertRefs(db, req, { property_id: b.property_id, unit_id: b.unit_id }))
...
router.put('/:id', validate(ExpenseSchema.partial()), (req, res) => {
  if (!canAccessExpense(db, req, req.params.id)) return res.status(404).json({ error: 'not_found' });
  if (!assertRefs(db, req, { property_id: req.body.property_id, unit_id: req.body.unit_id }))
    return res.status(404).json({ error: 'related_not_found' });
  const fields = [
    'property_id',
    'unit_id',
      ...
      if (s.marek_total && s.marek_total > 0) {
        if (!exists.get(chrobr.id, 'doplata', s.period)) {
          ins.run(
            ...
            s.marek_total,
            date,
            'Prowizja zarządcy (Marek) — z arkusza Excel',
          );
          inserted++;
        } else skipped++;
      }
