...
router.get('/', (req, res) => {
  ...
  if (req.user && req.user.id && req.user.role !== 'admin') {
    where.push('(t.owner_user_id = ? OR p.owner_user_id = ? OR up.owner_user_id = ? OR te.owner_user_id = ?)');
...
router.post('/', validate(TaskSchema), (req, res) => {
  const b = req.body;
  if (!assertRefs(db, req, { property_id: b.property_id, unit_id: b.unit_id, tenant_id: b.tenant_id })) return res.status(404).json({ error: 'related_not_found' });
  const r = db.prepare(`
...
router.put('/:id', validate(TaskSchema.partial()), (req, res) => {
  if (!canAccessTask(db, req, req.params.id)) return res.status(404).json({ error: 'not_found' });
  if (!assertRefs(db, req, { property_id: req.body.property_id, unit_id: req.body.unit_id, tenant_id: req.body.tenant_id })) return res.status(404).json({ error: 'related_not_found' });
