...
try {
  ...
  db.prepare("UPDATE units SET name = ?, base_rent = ?, base_media = ? WHERE code = ?").run('User Lokal', 4321, 987, 'KR');
  db.prepare("UPDATE recurring_costs SET amount = ?, notes = ? WHERE category = ? AND property_id IS NULL").run(999, 'User management', 'zarzadzanie');
  db.close();
  ...
  assert.equal(verify.prepare("SELECT value FROM settings WHERE key = ?").get('cost.management.monthly').value, '999');
  assert.equal(verify.prepare("SELECT district FROM properties WHERE name = ?").get('Kościelna 30/21').district, 'User district');
  ...
  const recurring = verify.prepare("SELECT amount, notes FROM recurring_costs WHERE category = ? AND property_id IS NULL").get('zarzadzanie');
  assert.equal(recurring.amount, 999);
  assert.equal(recurring.notes, 'User management');
  assert.equal(verify.prepare("SELECT COUNT(*) AS c FROM recurring_costs WHERE category = ? AND property_id IS NULL").get('zarzadzanie').c, 1);
  verify.close();
