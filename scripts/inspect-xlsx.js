...
const PL = ['styczeń','luty','marzec','kwiecień','maj','czerwiec','lipiec','sierpień','wrzesień','październik','listopad','grudzień'].map(normalize);
...
for (const sn of wb.SheetNames) {
  ...
  for (let r = 0; r < m.length; r++) {
    const row = m[r] || [];
    let first = null;
    for (const c of row) if (c != null && String(c).trim() !== '') { first = c; break; }
    if (findMonth(first) != null) {
      ...
      for (let rr = r; rr < end; rr++) {
        const cells = (m[rr] || []).map((c,i) => c == null ? '' : `${String.fromCharCode(65+i)}=${JSON.stringify(c).slice(0,28)}`).filter(Boolean).join('  ');
